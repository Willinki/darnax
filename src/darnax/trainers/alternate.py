...
class AltTrainer(Trainer[OrchestratorT, StateT], Generic[OrchestratorT, StateT]):
    """Minimal trainer that implements an alternated dynamical rule.
    ...
    def _train_step_impl(
        ...
        """Pure training core: free → clamped, then optimizer step.
        ...
        (state, rng), _ = scan_n(
            orchestrator.step, (state, rng), ctx["clamped_iter"], filter_messages="all"
        )  # clamped
        rng, back_key = jax.random.split(rng)

        # 4) second gradient computation
        grads = orchestrator.backward(state=state, rng=back_key)
