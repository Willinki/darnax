...
class ContrastiveHebbianTrainer(Trainer[OrchestratorT, StateT], Generic[OrchestratorT, StateT]):
    """Minimal trainer that implements a contrastive hebbian learning rule.

    This trainer wires together an orchestrator and a per-batch `State`,
    performing free/clamped dynamics. Only free
    ...
    During evaluation only free is performed.
    ...
    def _train_step_impl(
        ...
        """Pure training core: free -> clamped, then contrastive update.
        ...
        # 2) first free phase
        (sA, rng), _ = scan_n(
            orchestrator.step, (state, rng), ctx["free_iter"], filter_messages="left"
        )  # free
        # 2.1) first gradient computation
        rng, back_key = jax.random.split(rng)
        deltaA = orchestrator.backward(state=sA, rng=back_key)

        # 3) second clamped phase
        (sB, rng), _ = scan_n(
            orchestrator.step, (sA, rng), ctx["clamped_iter"], filter_messages="all"
        )  # clamped
        rng, back_key = jax.random.split(rng)
        # 3.1) second gradient computation
        deltaB = orchestrator.backward(state=sB, rng=back_key)

        # 4.1) difference between grads
        grads = jax.tree_util.tree_map(lambda a, b: a - b, deltaA, deltaB)

        # 5) update
        params = eqx.filter(orchestrator, eqx.is_inexact_array)  # trainable leaves
    ...
    def _eval_step_impl(
        ...
        """Pure evaluation core: free, then predict and compute metrics.
